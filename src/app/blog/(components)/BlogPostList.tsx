...
const BlogPostList = ({ data }: Props) => {
  ...
  return (
    <main className='flex flex-col gap-6 w-full'>
      ...
      {/* card list */}
      <ul
        className={`w-full ${viewType === 'card' && 'grid grid-cols-3 gap-10'} ${viewType === 'list' && 'flex flex-col gap-12 mobile:gap-5'} tablet_h:flex tablet_h:flex-col tablet_h:gap-10`}
      >
        {data.map(post => (
